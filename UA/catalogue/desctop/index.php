...
<main>
  <div class="main_rside-nav">
    <main>
      ...
      <div class="nav-collections">
        ...
        <a href="/UA/catalogue/desctop" class="nav-coll selected">Настільні світильники</a>
      </div>
      <div class="nav-news">
  ...
  <div class="content-wraper">
    <div class="content">
      ...
      switch ($sort) {
        case 'best-selling':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Desk Lamp' ORDER BY `rating` DESC");
        break;
        case 'price-ascending':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Desk Lamp' ORDER BY `cost_uah`");
        break;
        case 'price-descending':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Desk Lamp' ORDER BY `cost_uah` DESC");
        break;
        case 'created-descending':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Desk Lamp' ORDER BY `id` DESC");
        break;
        case 'created-ascending':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Desk Lamp' ORDER BY `id`");
        break;
        case 'A-Z':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Desk Lamp' ORDER BY `name`");
        break;
        case 'Z-A':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Desk Lamp' ORDER BY `name` DESC");
        break;
        default:
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Desk Lamp' ORDER BY `id`");
        break;
