...
<main>
  <div class="main_rside-nav">
    <main>
      ...
      <div class="nav-collections">
        ...
        <a href="/UA/catalogue/sconce" class="nav-coll selected">Бра</a>
  ...
  <div class="content-wraper">
    <div class="content">
      ...
      switch ($sort) {
        case 'best-selling':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Sconce' ORDER BY `rating` DESC");
        break;
        case 'price-ascending':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Sconce' ORDER BY `cost_uah`");
        break;
        case 'price-descending':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Sconce' ORDER BY `cost_uah` DESC");
        break;
        case 'created-descending':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Sconce' ORDER BY `id` DESC");
        break;
        case 'created-ascending':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Sconce' ORDER BY `id`");
        break;
        case 'A-Z':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Sconce' ORDER BY `name`");
        break;
        case 'Z-A':
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Sconce' ORDER BY `name` DESC");
        break;
        default:
        $products = $mysqli -> query("SELECT * FROM `goods` WHERE `type` = 'Sconce' ORDER BY `id`");
        break;
